...
class Delete extends Component {
	...
	getGroupInfoFromCloud(groupId) {
		...
		firebase.database().ref('groupsinfo/'+groupId).on('value',callback);
	}
	...
	deleteGroup(groupId) {
		...
		//bulk update---->atomic update
		firebase.database().ref('groupsinfo').child(groupId).remove();
		firebase.database().ref('messageInfo').child(groupId).remove();
		firebase.database().ref('users1/'+userUid+"/usergroupkeys").child(groupId).remove();

		console.log("the group and its messages are deleted")
       					  	...
       					  	return (
       					  		<div>
	       					  		...
	       					  		<li className ="yellow collection-item" key={i}>
	       					  			...
	       					  			<div >{group.groupname}
	       					  			<a onMouseOver="this.innerHTML='this is awsome';"onClick={()=>{this.deleteGroup(group.newGroupCreatedKey)}} className="secondary-content"><i className="material-icons">
	       					  			clear</i>
	       					  			</a>
	       					  			</div>
	       					  		</li>
